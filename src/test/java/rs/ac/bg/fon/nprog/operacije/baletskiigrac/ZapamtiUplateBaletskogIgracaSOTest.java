...
class ZapamtiUplateBaletskogIgracaSOTest extends ApstraktnaSOTest{
	
	ZapamtiUplateBaletskogIgracaSO operacija;
	
	@BeforeEach
	void setUp() throws Exception {
		operacija=new ZapamtiUplateBaletskogIgracaSO(repository);
	}
	...
	void testExecuteOperation() {
		try {
			...
			when(repository.pronadji(baletskiIgrac.getListaUplata().get(0))).thenReturn(lista);
						
			operacija.execute(baletskiIgrac);
			verify(repository, times(1)).pronadji(uplata1);
			verify(repository, times(1)).daLiPostoji(uplata1);
