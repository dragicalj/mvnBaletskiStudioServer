...
class PronadjiBaletskeIgraceSOTest extends ApstraktnaSOTest{
	
	PronadjiBaletskeIgraceSO operacija;
	
	@BeforeEach
	void setUp() throws Exception {
		operacija=new PronadjiBaletskeIgraceSO(repository);
	}
	...
	void testExecuteOperation() {
		try {
			...
			when(repository.pronadji(any())).thenReturn(listaIgraca);
			operacija.execute(baletskiIgrac1);
			verify(repository, times(1)).pronadji(any());
